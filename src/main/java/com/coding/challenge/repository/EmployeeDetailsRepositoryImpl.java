...
public class EmployeeDetailsRepositoryImpl implements EmployeeDetailsRepository {
	...
	public Set<EmployeeDetails> findAll() {
		return inMemoryDatabase.stream().sorted(Comparator.comparing(EmployeeDetails::getEmployeeId)).collect(Collectors.toSet());
	}
	...
	public Optional<EmployeeDetails> findByEmployeeId(Long employeeId) {
		...
		List<EmployeeDetails> employeeList = inMemoryDatabase.stream().filter(e -> employeeId.equals(e.getEmployeeId()))
	...
	public List<EmployeeDetails> findAllByManagerId(Long managerId) {
		...
		if (managerId != null) {
			return inMemoryDatabase.stream()
					.filter(e -> (e.getManagerId() != null && managerId.equals(e.getManagerId())))
					.sorted(Comparator.comparing(EmployeeDetails::getEmployeeId)).collect(Collectors.toList());
		} else {
			return inMemoryDatabase.stream().filter(e -> e.getManagerId() == null)
					.sorted(Comparator.comparing(EmployeeDetails::getEmployeeId)).collect(Collectors.toList());
		}
