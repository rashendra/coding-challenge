...
var app = angular
		.module(
				...
				[ 'ui.bootstrap', 'ngResource', 'ngRoute', 'hljs',
						'spring-data-rest' ])
		...
		.controller(
				...
				function($scope, $http, SpringDataRestAdapter, $filter) {
					...
					$scope.loadEmployeeHierarchy = function() {
						...
						$http.post('/api/v0/employeeDetails/fetchEmpHierarchy',
								request, {
									headers : {
										'API-KEY' : 'xJ9a34fo'
									}
					...
					$scope.initialize = function() {
						...
						var httpPromise = $http.get(
								'/api/v0/employeeDetails/initialize').success(
								function(response) {
									console.log("initialize");
									$scope.fetchAll();
								});
					...
					$scope.fetchAll = function() {
						...
						var httpPromise = $http
								.get('/api/v0/employeeDetails/fetchAll')
								.success(
										function(response) {
					...
					$scope.clearData = function() {
						var httpPromise = $http.get(
								'/api/v0/employeeDetails/clearDb').success(
								function(response) {
									$scope.fetchAll();
								});
					...
					$scope.saveEmpDetails = function(isValid) {
						if ($scope.isManager
								|| $scope.employeeIdList
										.indexOf($scope.employeeDetails.managerId) !== -1) {

							$http.post('/api/v0/employeeDetails/create',
									$scope.employeeDetails, {
										headers : {
											'API-KEY' : 'xJ9a34fo'
										}
				...
				function($compile) {
					return {
						...
						link : function(scope, element, attrs) {
							...
							console.log(parent);
							var template = ' <table class="table table-striped table-bordered" id="nested-table">'
									+ '<tr  style="width:100%; height:30%;">'
									+ '<td style="width:50%">{{ '
									+ parent
									+ '.employeeName  }}</td>'
									+ '</tr>'
									+ '<tr style="width:100%" ng-repeat="sub in '
									+ parent
									+ '.subs">'
